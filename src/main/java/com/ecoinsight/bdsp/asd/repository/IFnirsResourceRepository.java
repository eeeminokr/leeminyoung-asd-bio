...
public interface IFnirsResourceRepository extends IEntityRepository {

    class FnirsResourceSqlProvider implements ProviderMethodResolver {
        ...
        public String findAll(long projectSeq, int trialIndex, String subjectId, String orgId, String gender,
            ...
            return new SQL() {
                { // CASE WHEN PP.Completed = 0 THEN 'NO_DATA' ELSE CAST(PP.Completed AS CHAR) END
                    ...
                    LEFT_OUTER_JOIN("T_FNIRsResource PP ON PP.SubjectId = T.SubjectId AND PP.TrialIndex = T.TrialIndex AND PP.ProjectSeq = T.ProjectSeq AND PP.TrialIndex = T.TrialIndex AND PP.ProjectSeq = T.ProjectSeq");
    ...
    public List<FnirsResource> queryFnirsResources(@Param("systemId") String systemId,
    ...
    public List<FnirsResource> queryByPublished(@Param("hostname") String hostname,
            @Param("published") boolean published);

    @SelectProvider(FnirsResourceSqlProvider.class)
    public List<FnirsResource> findAll(@Param("systemId") String systemId, @Param("subjectId") String subjectId,
    ...
    @SelectProvider(FnirsResourceSqlProvider.class)
    public List<FnirsResource> countAll(@Param("systemId") String systemId, @Param("subjectId") String subjectId,
            @Param("projectSeq") long projectSeq, @Param("trialIndex") int trialIndex,
            @Param("orgId") String orgId,
            @Param("gender") String gender, @Param("page") int page,
            @Param("offset") int offset);
    ...
    public List<FnirsResource> findByProjectSeqAndTrialIndex(@Param("systemId") String systemId,
            @Param("subjectId") String subjectId, @Param("projectSeq") long projectSeq,
            @Param("trialIndex") int trialIndex);

    @Insert("INSERT INTO T_FNIRsResource (Id, OrgId, SystemId, ProjectSeq, SubjectId, TrialIndex, OriginalFileName, TargetFileName, Hostname, Published, Completed, Deleted, CDate, CUser, UDate, UUser)"
    ...
    public void add(FnirsResource resource);

    @Update("UPDATE T_FNIRsResource "
            ...
            + " WHERE Id=#{id}")
    public boolean changeAfterPublishedAndLabelled(FnirsResource resource);

    @Update("UPDATE T_FNIRsResource "
            + " SET  ProjectSeq = #{projectSeq}"
            + " WHERE Id=#{id}")
    public boolean updateSubjectProjectSeq(FnirsResource resource);


    @Delete("DELETE FROM T_FNIRsResource E "
