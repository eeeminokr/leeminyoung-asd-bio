...
@Transactional
@Service(IFnirsResourceService.ID)
public class IFnirsResourceService implements AsdSubjectService {

    private final Logger LOGGER = LoggerFactory.getLogger(getClass());
    public static final String ID = "IFnirsResourceService";
    ...
    public void create(AsdProjectSubject subject, FnirsResource resource) throws Exception {
        try {
            this._fNIRsResourceRepository.add(resource);
        } catch (Exception e) {
        ...
        try {
            ...
            List<FnirsResource> resources = this._fNIRsResourceRepository.queryFnirsResources(
                    resource.getSystemId(), resource.getSubjectId(),
                    resource.getProjectSeq(), resource.getTrialIndex());
    ...
    public boolean download(File downloadDir, String systemId, String subjectId, long projectSeq, int trialIndex)
        ...
        List<FnirsResource> resources = this._fNIRsResourceRepository.findByProjectSeqAndTrialIndex(systemId, subjectId,
        ...
        for (FnirsResource fresource : resources) {
            LocalDateTime ptime = fresource.getDatePublished();
            LocalDateTime cptime = fresource.getDatePublished();
            ...
            String fname = fresource.getTargetFileName();
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public void delete(List<FnirsResource> resources, FnirsResource model, String worker) throws Exception {
        ...
        try {
            ...
            final AmazonS3 s3 = AmazonS3ClientBuilder.standard()
                    ...
                    .withCredentials(new AWSStaticCredentialsProvider(credentials))
                    .build();
            for (FnirsResource resource : resources) {
                ...
                if (this._fNIRsResourceRepository.delete(resource.getSubjectId(), resource.getProjectSeq(),
    ...
    public List<FnirsResource> getFnirsResource(String systemId, String subjectId, long projectSeq, long subjectProjectSeq, int trialIndex) {

        // long subSetProjectSeq = subject.getProjectSeq();
        // if(model.getProjectSeq()!= subject.getProjectSeq()){
        //     subProjectSeq  = subject.getProjectSeq();
        // }
        List<FnirsResource> list = this._fNIRsResourceRepository.queryFnirsResources(systemId, subjectId, projectSeq, trialIndex);
        boolean state = false;
        for (FnirsResource fnirs : list) {
            if (fnirs.getProjectSeq() != subjectProjectSeq) {
                state = this._fNIRsResourceRepository.updateSubjectProjectSeq(fnirs);
            }

        }

        List<FnirsResource> updateList = new ArrayList<>();
        if (state) {
            updateList = this._fNIRsResourceRepository.queryFnirsResources(systemId, subjectId, subjectProjectSeq, trialIndex);
        } else {
            updateList = list;
        }
