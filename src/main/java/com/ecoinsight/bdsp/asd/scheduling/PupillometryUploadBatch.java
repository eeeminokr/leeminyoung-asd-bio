...
@Component
public class PupillometryUploadBatch extends AsdBaseScheduler {
    ...
    @Autowired
    protected IPupillometryRepository _pupillometryRepository;
    @Autowired
    private DataCommonService _dataCommonService;

    @PostConstruct
    public void startup() throws UnknownHostException {
        this.taskName = "PupillometryUploadBatch";
    ...
    public void doPerform() {
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("-> PupillometryUploadBatch.doPerform.... enabled=" + this.enabled);
        }
        ...
        List<Pupillometry> resources = this._pupillometryRepository.queryByPublished(this._hostname, Boolean.TRUE);
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("-> queryByPublished. - result=" + resources.size());
        }
        boolean status = true;
        for (Pupillometry vr : resources) {
            ...
            if (!(file.exists()) && !(vr.isCompleted())) {
                ...
                this._pupillometryRepository.changeAfterPublishedAndLabelled(vr);
