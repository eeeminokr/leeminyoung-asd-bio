<template>
    <div id="ContentsDiv">
                ...
                <ul class="list_input">
                    ...
                    <li class="btns fr">
                        <template v-if="canQuery">
                            <button class="btn btn_violet" type="button" @click="searchFnirsResourceyData">검색</button>
                                ...
                                <table class="tb_head">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th rowspan="2" class="sticky">fNIRS</th>
                                ...
                                <table class="tb_body">
                                    ...
                                    <tbody>
                                        <tr v-for="(item, index) in subjects" :key="item.subjectId">
                                            ...
                                            <td><img v-if="canQueryImaging" :src="`${getQcStatusIcon('FNIRS', item)}`">
                                            </td>
        ...
        <transition>
            ...
            <fnirs-resource-delete-modal @closed="onCloseDeleteModal" @check="onCheckModal"
            v-if="showCanDeleteOptionModal" :item=this.item> 

            </fnirs-resource-delete-modal>
        </transition>
        <vue-element-loading 
			:active="working" 
...
import memberService from '../../services/member.service';
import FnirsResourceDeleteModal from '../../components/FnirsResourceDeleteModal.vue';
import FnirsResourceService from '../../services/fnirsresource.service';

const ROWS = 20;
const FUNCTION_NAME = FUNCTION_NAMES.TRIAL_BIOMARKER_FNIRS;
...
export default {
    ...
    methods: {
        isDoneQCData(data) {
            console.log("item['areaQcStatus'] : " + data.FNIRS)
            if (data.FNIRS === 'DONE_QC') {
                return true;
            }
        ...
        openCheckDeleteOption(data) {
            console.log("삭제하기 method" + data['areaQcStatus'].FNIRS)
            if ((data['areaQcStatus'].FNIRS === 'DONE_QC')) {
        ...
        searchFnirsResourceyData() {
            ...
            MemberService.getAuthMemberProjectPermission(this.projectId, FUNCTION_NAME).then(result => {
                if (result.data.succeeded) {
                    ...
                    TrialService.searchFnirsResourceResource(this.filters).then(result => {
                        if (result.data.succeeded) {
                            this.subjects = result.data.data.items;
                            this.paging = { totalCount: result.data.data.total, pageNo: this.filters.page };
                            this.pupillometryDataCount = result.data.data.total;
        ...
        download(item) {
            ...
            console.log(item)
            FnirsResourceService.download(item['subjectId'], item['projectSeq'], item['trialIndex']).then(response => {
        ...
        dataSetDownload(){
            ...
            FnirsResourceService.dataSetDownload(projectSeq).then(response => {
