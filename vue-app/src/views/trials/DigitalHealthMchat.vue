<template>
    <div id="ContentsDiv">
                ...
                <li>
                  ...
                  <li class="btns fr">
                      <template v-if="canQuery">
                        <button class="btn btn_violet" type="button" @click="searchMchatData">검색</button>
      ...
      <div class="cont_body mt_11">
        ...
        <tbody><tr><td>
            <div class="scroll_head">
                <table class="tb_head">
                    <colgroup>
                        ...
                        <col width="120px"><!-- 연구번호ID -->
                        <col width="50px"><!-- 성별 -->
                        <col width="80px"><!-- 연령 -->
                        <col width="120px"><!-- 참여기관 -->
                        <col width="120px"><!-- 검사수행일 -->
                        <col width="70px"><!-- 대상군 -->
    
                        <col width="50px" v-for="n in MCHAT_ANSWER_NUMBER" :key="n">                    
                    </colgroup>
                    <thead>
                        <tr>
                            <th rowspan="2" class="sticky"><label>
                            ...
                            <th rowspan="2" class="sticky border_dark">대상군</th>
                            <th :colspan=MCHAT_ANSWER_NUMBER>M-CHAT</th>
                        </tr>                
                        <tr>
                            <th v-for="col in MCHAT_ANSWER_NUMBER">{{col}}</th>
                        </tr>
                    </thead>
                </table>
            </div>
            <div class="scroll_body ps ps--active-x ps--active-y" style="max-height: 605px;">
                <table class="tb_body">
                    ...
                    <tbody>
                        <template v-for="item in subjects" :key="item.subjectId">
                            <!-- <tr v-for="data in item.mcdData" :key="data['_ID']"> -->
                            <tr>    
                                <td class="sticky">
                                    <!-- <label v-if="canDelete"><input type="checkbox" name="table_check" v-model="selectedItems" :id="data['_ID']" :value="data" /><span></span></label> -->
                                    <label>
                                        <!-- <input type="checkbox" name="table_check" v-model="selectedItems" :id="item.subjectId" :value="item" /> -->
                                        <span></span></label>
                                </td>
                                <td class="sticky">{{item.subjectId}}</td>
                                <td class="sticky">{{gender(item.gender)}}</td>
                                <td class="sticky">{{item.months}} 개월</td>
                                <td class="sticky">{{item.orgName}}</td>
                                <td class="sticky">{{item.mchatCreatedDate}}</td>
                                <td class="sticky border_dark">{{item.projectName}}</td>
                                <td v-if="item.months <= 30" v-for="n in MCHAT_ANSWER_NUMBER" :key="n">
                                    {{getMchatAnswer(item, n)}}
                                </td>
                                <td v-if="item.months > 30" :colspan="MCHAT_ANSWER_NUMBER">
                                    대상아님
                                </td>                 
                            </tr>
                        </template>
                        <template v-if="subjects.length <= 0">
                            <tr >
                                <td :colspan="6 + MCHAT_ANSWER_NUMBER">
                                    <p>데이터가 존재하지 않습니다</p>
                                </td>              
                            </tr>
                        </template>
        ...
        <!-- <list-paging-with-page-count 
                        @click="onPagingClick" 
                        @offsetChanged="onOffsetChanged"
                        :paging="paging" 
                        :rowsPerPage="filters.offset"
                        :subjectCount="mchatDataCount"></list-paging-with-page-count> -->
      </div>
      <vue-element-loading 
			:active="working" 
    ...
    import VueElementLoading from "vue-element-loading";
        ...
        const ROWS = 20;
        const FUNCTION_NAME = FUNCTION_NAMES.TRIAL_DIGITALHEALTH_MCHAT;
        const GROUP_NAME = 'T_MChart';
        
    
        
    
        export default {
            name: "TRIAL_DIGITALHEALTH_MCHAT ",
            props: {},
            components: {
                VueElementLoading,
                TrialDataTab,
            ...
            data() {
                return {
                    MCHAT_ANSWER_NUMBER: 23,
                    ORG_ID: AuthService.getLoginUserOrganization(),
                    MENU_ID: 'TRIAL',
                    SUBMENU_ID: 'TRIAL_DIGITALHEALTH_MCHAT',
                    projectId: undefined,
                    projects: [],
                    organizations: [],
                    subjects: [],
		...
		workingMessage: '다운로드할 파일을 생성 중입니다...',  
            ...
            methods: {
                ...
                deleteSelectedItems() {
                    ...
                    this.selectedItems.map(val=>{ list.push ({
                        projectId: projectId, 
                        trialIndex: val['trialIndex'], 
                        subjectId: val['subjectId']}); });
                    this.selectedItems = [];
                    
                    
                    //delete 로직 변경할 부분 있으면 변경할 것
                    // TrialService.deleteTrialDatas(list, [GROUP_NAME]).then(results=>{
                    //     let count = 0;
                    //     results.map(r=>{
                    //         if (r.data.succeeded) count++;
                    //     });
    
                    //     this.searchMchatData();
               ...
               if(this.vaildgender){
                ...
                getMchatAnswer(item, index){
                 
                    if(item.result !=null){
                        let mchatArr = item.result.split(',');
                        // console.debug(mchatArr);
                        // console.debug(mchatArr[index-1])
                        return mchatArr[index-1]
                    }else{
                        return "-"
                    }
                },
                getMonthsBetween(birthDay, referenceDate, createDate){
                    return referenceDate!=null?utils.getMonthsBetween(birthDay, referenceDate):utils.getMonthsBetween(birthDay, createDate);
                },
                resetTrialData() {
      ...
      AlertService.info(						
            ...
            TrialService.dataSetDownload(projectSeq).then(response => {
        ...
        AlertService.info(						
            ...
            created() {
				...
				if (result.data.succeeded) {
                    ...
                    console.log("rolePermission[mchat]==> ",this.rolePermission)
                    this.getProjects();
