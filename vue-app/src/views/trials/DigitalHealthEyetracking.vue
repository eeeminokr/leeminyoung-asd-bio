<template>
    <div id="ContentsDiv">
                ...
                <ul class="list_input">
                    ...
                    <li class="btns fr">
                        <template v-if="canQuery">
                            <button class="btn btn_violet" type="button" @click="searchEyeTrackingData">검색</button>
                                ...
                                <table class="tb_head">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th rowspan="2" class="sticky">이미지</th>
                                            <th rowspan="2" class="sticky">동영상</th>
                                ...
                                <table class="tb_body">
                                    ...
                                    <tbody>
                                        <tr v-for="(item, index) in subjects" :key="item.subjectId">
                                            ...
                                            <td><img v-if="canQueryImaging" :src="`${getQcStatusIcon('IMAGE', item)}`">
                                            </td>
                                            <td><img v-if="canQueryImaging" :src="`${getQcStatusIcon('VIDEO', item)}`">
                                            </td>
        ...
        <transition>
            <eye-tracking-delete-modal @closed="onCloseDeleteModal" @check="onCheckModal"
                v-if="showCanDeleteOptionModal" :item=this.item>

            </eye-tracking-delete-modal>
        </transition>
        <vue-element-loading 
			:active="working" 
...
import memberService from '../../services/member.service';
import EyeTrackingDeleteModal from '../../components/EyeTrackingDeleteModal.vue';
import EyetrackingService from '../../services/eyetracking.service';

const ROWS = 20;
const FUNCTION_NAME = FUNCTION_NAMES.TRIAL_DIGITALHEALTH_EYETRACKING;
...
export default {
    ...
    methods: {
        isDoneQCData(data) {
            ...
            if ((data.IMAGE === 'DONE_QC' || data.VIDEO === 'DONE_QC')) {
                return true;
            }
        ...
        openCheckDeleteOption(data) {
            console.log("삭제하기 method")
            if ((data['areaQcStatus'].IMAGE === 'DONE_QC' || data['areaQcStatus'].VIDEO === 'DONE_QC')) {
        ...
        onCloseDeleteModal(data) {
            ...
            console.log("subjectId :", data.data.data[0].subjectId)
            ...
            if (data.data.succeeded) {
                ...
                setTimeout(() => {
                    this.filters.subjectId = data.data.data[0].subjectId
                    this.reloadCheck()
                }, 2000);
            } else {
        ...
        searchEyeTrackingData() {
            ...
            MemberService.getAuthMemberProjectPermission(this.projectId, FUNCTION_NAME).then(result => {
                if (result.data.succeeded) {
                    ...
                    TrialService.searchEyeTrackingResource(this.filters).then(result => {
                        if (result.data.succeeded) {
                            ...
                            if (currentPage === totalPages && Object.keys(groupedData).length % pageSize !== 0) {
                                end = start + (Object.keys(groupedData).length % pageSize);
        ...
        download(item) {
            ...
            EyetrackingService.download(item['subjectId'], item['projectSeq'], item['trialIndex']).then(response => {
        ...
        dataSetDownload(){
            ...
            EyetrackingService.dataSetDownload(projectSeq).then(response => {
    ...
    created() {
        ...
        MemberService.getAuthMemberFunctionPermission(FUNCTION_NAME).then(result => {
            if (result.data.succeeded) {
                ...
                console.log("rolePermission[eye]==> ", this.rolePermission)
                this.getProjects();
